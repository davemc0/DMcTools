...
void PrintFormatNumber(void* ValuePtr, int Format, int ByteCount)
{
    ...
    for (n = 0; n < 16; n++) {
        ...
        case FMT_DOUBLE:
            ...
            s = 8;
            break;
        default: printf("Unknown format %d:", Format); return;
        }
        ByteCount -= s;
...
double ConvertAnyFormat(void* ValuePtr, int Format)
{
    ...
    default: ErrNonfatal("Illegal format code %d in Exif header", Format, 0);
    }
    return Value;
}
...
#define DIR_ENTRY_ADDR(Start, Entry) (Start + 2 + 12 * (Entry))

    {
        ...
        if (DumpExifMap) { printf("Map: %05d-%05d: Directory\n", (int)(DirStart - OffsetBase), (int)(DirEnd + 4 - OffsetBase)); }
    }

    if (ShowTags) { printf("(dir has %d entries)\n", NumDirEntries); }
    ...
    for (de = 0; de < NumDirEntries; de++) {
        ...
        if (ByteCount > 4) {
            ...
            if (DumpExifMap) { printf("Map: %05d-%05d:   Data for tag %04x\n", OffsetVal, OffsetVal + ByteCount, Tag); }
        } else {
        ...
        if (Tag == TAG_MAKER_NOTE) {
            if (ShowTags) { printf("%s    Maker note: ", IndentString); }
        ...
        switch (Tag) {
        case TAG_MAKE: strncpy(ImageInfo.CameraMake, (char*)ValuePtr, ByteCount < 31 ? ByteCount : 31); break;

        case TAG_MODEL: strncpy(ImageInfo.CameraModel, (char*)ValuePtr, ByteCount < 39 ? ByteCount : 39); break;
        ...
        case TAG_MAXAPERTURE:
            ...
            if (ImageInfo.ApertureFNumber == 0) { ImageInfo.ApertureFNumber = (float)exp(ConvertAnyFormat(ValuePtr, Format) * log(2) * 0.5); }
            break;
        ...
        case TAG_SHUTTERSPEED:
            ...
            if (ImageInfo.ExposureTime == 0) { ImageInfo.ExposureTime = (float)(1 / exp(ConvertAnyFormat(ValuePtr, Format) * log(2))); }
            break;

        case TAG_FLASH: ImageInfo.FlashUsed = (int)ConvertAnyFormat(ValuePtr, Format); break;
        ...
        case TAG_ORIENTATION:
            ...
            if (NumOrientations == 0) { ImageInfo.Orientation = (int)ConvertAnyFormat(ValuePtr, Format); }
        ...
        case TAG_FOCAL_PLANE_XRES: FocalplaneXRes = ConvertAnyFormat(ValuePtr, Format); break;
        ...
        case TAG_EXPOSURE_BIAS: ImageInfo.ExposureBias = (float)ConvertAnyFormat(ValuePtr, Format); break;

        case TAG_WHITEBALANCE: ImageInfo.Whitebalance = (int)ConvertAnyFormat(ValuePtr, Format); break;

        case TAG_LIGHT_SOURCE: ImageInfo.LightSource = (int)ConvertAnyFormat(ValuePtr, Format); break;

        case TAG_METERING_MODE: ImageInfo.MeteringMode = (int)ConvertAnyFormat(ValuePtr, Format); break;

        case TAG_EXPOSURE_PROGRAM: ImageInfo.ExposureProgram = (int)ConvertAnyFormat(ValuePtr, Format); break;
        ...
        case TAG_EXPOSURE_MODE: ImageInfo.ExposureMode = (int)ConvertAnyFormat(ValuePtr, Format); break;

        case TAG_ISO_EQUIVALENT: ImageInfo.ISOequivalent = (int)ConvertAnyFormat(ValuePtr, Format); break;

        case TAG_DIGITALZOOMRATIO: ImageInfo.DigitalZoomRatio = (float)ConvertAnyFormat(ValuePtr, Format); break;
...

    {
        ...
        if (DIR_ENTRY_ADDR(DirStart, NumDirEntries) + 4 <= OffsetBase + ExifLength) {
            ...
            if (Offset) {
                ...
                if (Offset > ImageInfo.LargestExifOffset) { ImageInfo.LargestExifOffset = Offset; }
            }
        } else {
    ...
    if (ThumbnailOffset) {
        ...
        if (ThumbnailOffset <= ExifLength) {
            ...
            if (ShowTags) { printf("Thumbnail size: %d bytes\n", ThumbnailSize); }
        }
    }
}
...
void process_EXIF(unsigned char* ExifSection, unsigned int length)
{
    ...
    if (ShowTags) { printf("Exif header %d bytes long\n", length); }
...
void create_EXIF(void)
{
...

    {
        ...
        if (NewBuf == NULL) { ErrFatal("Could not allocate memory"); }
...
const char* ClearOrientation(void)
{
    ...
    for (a = 0; a < NumOrientations; a++) {
        ...
        case FMT_SBYTE:
        case FMT_BYTE: *(uchar*)(OrientationPtr[a]) = 1; break;

        case FMT_USHORT: Put16u(OrientationPtr[a], 1); break;
...
int Exif2tm(struct tm* timeptr, char* ExifTime)
{
    ...
    timeptr->tm_sec = 0;
    a = sscanf(ExifTime, "%d%*c%d%*c%d%*c%d:%d:%d", &timeptr->tm_year, &timeptr->tm_mon, &timeptr->tm_mday, &timeptr->tm_hour, &timeptr->tm_min,
               &timeptr->tm_sec);
...
void ShowImageInfo(int ShowFileInfo)
{
    ...
    if (ImageInfo.DateTime[0]) { printf("Date/Time    : %s\n", ImageInfo.DateTime); }
    ...
    if (ImageInfo.IsColor == 0) { printf("Color/bw     : Black and white\n"); }
    ...
    if (ImageInfo.FocalLength) {
        ...
        if (ImageInfo.FocalLength35mmEquiv) { printf("  (35mm equivalent: %dmm)", ImageInfo.FocalLength35mmEquiv); }
        printf("\n");
    }
    ...
    if (ImageInfo.CCDWidth) { printf("CCD width    : %4.2fmm\n", (double)ImageInfo.CCDWidth); }
    ...
    if (ImageInfo.ExposureTime) {
        ...
        if (ImageInfo.ExposureTime <= 0.5) { printf(" (1/%d)", (int)(0.5 + 1 / ImageInfo.ExposureTime)); }
        printf("\n");
    }
    ...
    if (ImageInfo.ISOequivalent) { printf("ISO equiv.   : %2d\n", (int)ImageInfo.ISOequivalent); }
    ...
    case 1: printf("Whitebalance : Manual\n"); break;
    case 0: printf("Whitebalance : Auto\n"); break;
    }
    ...
    if (ImageInfo.QualityGuess) { printf("JPEG Quality : %d\n", ImageInfo.QualityGuess); }
...
void ShowConciseImageInfo(void)
{
    ...
    if (ImageInfo.ApertureFNumber) { printf(" f/%3.1f", (double)ImageInfo.ApertureFNumber); }

    if (ImageInfo.FocalLength35mmEquiv) { printf(" f(35)=%dmm", ImageInfo.FocalLength35mmEquiv); }

    if (ImageInfo.FlashUsed >= 0 && ImageInfo.FlashUsed & 1) { printf(" (flash)"); }

    if (ImageInfo.IsColor == 0) { printf(" (bw)"); }
