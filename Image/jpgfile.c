...
//--------------------------------------------------------------------------
static int Get16m(const void* Short) { return (((uchar*)Short)[0] << 8) | ((uchar*)Short)[1]; }
...
{
    if (SectionsRead > SectionsAllocated) { ErrFatal("allocation screwup"); }
    if (SectionsRead >= SectionsAllocated) {
        ...
        if (Sections == NULL) { ErrFatal("could not allocate data for entire image"); }
    }
}
...
int ReadJpegSections(FILE* infile, ReadMode_t ReadMode)
{
    ...
    for (;;) {
        ...
        for (a = 0;; a++) {
            ...
            if (marker == EOF) { ErrFatal("Unexpected end of file"); }
            prev = marker;
        }

        if (a > 10) { ErrNonfatal("Extraneous %d padding bytes before section %02X", a - 1, marker); }
        ...
        lh = fgetc(infile);
        ll = fgetc(infile);
        if (lh == EOF || ll == EOF) { ErrFatal("Unexpected end of file"); }

        itemlen = (lh << 8) | ll;

        if (itemlen < 2) { ErrFatal("invalid marker"); }

        Sections[SectionsRead].Size = itemlen;

        Data = (uchar*)malloc(itemlen);
        if (Data == NULL) { ErrFatal("Could not allocate memory"); }
        ...
        if (got != itemlen - 2) { ErrFatal("Premature end of file?"); }
        SectionsRead += 1;
        ...
        case M_SOS: // stop before hitting compressed data
            ...
            if (ReadMode & READ_IMAGE) {
                ...
                if (Data == NULL) { ErrFatal("could not allocate data for entire image"); }

                got = fread(Data, 1, size, infile);
                if (got != size) { ErrFatal("could not read the rest of the image"); }
        ...
        case M_JFIF:
            ...
            if (memcmp(Data + 2, "JFIF\0", 5)) { fprintf(stderr, "Header missing JFIF marker\n"); }
            if (itemlen < 16) {
            ...
            if (ShowTags) {
                ...
                if (Data[14] || Data[15]) { fprintf(stderr, "Ignoring jfif header thumbnail\n"); }
            }
            ...
            if (ReadMode & READ_METADATA) {
                if (memcmp(Data + 2, "Exif", 4) == 0) {
                    ...
                    if (ShowTags) {
                        ...
                        if (ShowTags) { ShowXmp(Sections[SectionsRead - 1]); }
                    }
                    break;
                }
            }
        ...
        case M_IPTC:
            if (ReadMode & READ_METADATA) {
                if (ShowTags) { printf("Image cotains IPTC section, %d bytes long\n", itemlen); }
...
int ReplaceThumbnail(const char* ThumbFileName)
{
    ...
    if (ThumbFileName) {
        ...
        if (ThumbLen + ImageInfo.ThumbnailOffset > 0x10000 - 20) { ErrFatal("Thumbnail is too large to insert into exif header"); }
    } else {
        if (ImageInfo.ThumbnailSize == 0) { return FALSE; }
...
void WriteJpegFile(const char* FileName)
{
    ...
    if (!HaveAll) { ErrFatal("Can't write back - didn't read all"); }

    outfile = fopen(FileName, "wb");
    if (outfile == NULL) { ErrFatal("Could not open file for write"); }
    ...
    if (Sections[0].Type != M_EXIF && Sections[0].Type != M_JFIF) {
        ...
        static uchar JfifHead[18] = {0xff, M_JFIF, 0x00, 0x10, 'J', 'F', 'I', 'F', 0x00, 0x01, 0x01, 0x01, 0x01, 0x2C, 0x01, 0x2C, 0x00, 0x00};
...
Section_t* CreateSection(int SectionType, unsigned char* Data, int Size)
{
    ...
    if (SectionsRead < NewIndex) { ErrFatal("Too few sections!"); }

    CheckSectionsAllocated();
    for (a = SectionsRead; a > NewIndex; a--) { Sections[a] = Sections[a - 1]; }
    SectionsRead += 1;
