...
/*******************************************/
static inline bool bmpError(const char *fname, const char *st)
{
	cerr << fname << ": " << st << endl;
	return false;
}
		...
		while (y<(int)h) {
			...
			if(c) { /* encoded mode */
				...
				if(c == 0x00) { /* end of line */
					x=0; y++; pp = Pix + x + (h-y-1)*w;
				}
				
				else if(c == 0x01) break; /* end of Pix */
		...
		for(i=h-1; i>=0; i--)
		  {
		    ...
		    fread(pp, 1, w, fp);
		    for(int j=w; j<padw; j++)
		      getc(fp);

		    if(FERROR(fp)) {rv = 1; break;}
		  }
	}
...
bool ImageLoadSave::LoadBMP(const char *fname)
{
	...
	if(biBitCount<24) {
...
#ifdef DMC_DEBUG
		{
			...
			for(i=0; i<cmaplen; i++) {
				fprintf(stderr,"%02x%02x%02x ", red[i],grn[i],blu[i]);
			}
			fprintf(stderr,"\n\n");
		}
#endif
	...
	wid = biWidth; hgt = biHeight; is_uint = false; is_float = false;
...
/*******************************************/
bool ImageLoadSave::SaveBMP(const char *fname) const
{
	...
	if(chan == 1) {
		for(i=0; i<nc; i++) {
			putc(i,fp); putc(i,fp); putc(i,fp); putc(0,fp);
		}
	}
	...
	else bmpError(fname, "Bad bit depth");
	
	if(FERROR(fp)) return DMC_ERROR;
