...
static int decode_map8(const unsigned char* src0, unsigned char* dest0, const int size, const int chan, const unsigned char* color_map)
{
    const unsigned char* src = src0;
    unsigned char* dest = dest0;

    for (int i = 0; i < size; i++) {
        for (int c = 0; c < chan; c++) dest[chan - c - 1] = color_map[src[0] * chan + c];
        src++;
        dest += chan;
    }
...
static int decode_rgb24(const unsigned char* src0, unsigned char* dest0, const int wid, const int hgt)
{
    const unsigned char* src = src0;
    unsigned char* dest = dest0;
...
static int decode_rgb32(const unsigned char* src0, unsigned char* dest0, const int wid, const int hgt)
{
    const unsigned char* src = src0;
    unsigned char* dest = dest0;
...
static int decode_rgb_rle16(const unsigned char* src0, unsigned char* dest0, const int wid, const int hgt, const bool R5G6B5)
{
    int chan = 3;
    const unsigned char* src = src0;
    unsigned char* dest = dest0;
...
static int decode_rgb_rle8(const unsigned char* src0, unsigned char* dest0, const int wid, const int hgt)
{
    int chan = 1;
    const unsigned char* src = src0;
    unsigned char* dest = dest0;
...
static int decode_rgb_rle24(const unsigned char* src0, unsigned char* dest0, const int wid, const int hgt)
{
    int chan = 3;
    const unsigned char* src = src0;
    unsigned char* dest = dest0;
...
static int decode_rgb_rle32(const unsigned char* src0, unsigned char* dest0, const int wid, const int hgt)
{
    int chan = 4;
    const unsigned char* src = src0;
    unsigned char* dest = dest0;
...
void ImageLoadSave::LoadTGA(const char* fname, bool R5G6B5)
{
    ...
    if (InFile.gcount() != fsize) {
        InFile.close();
        throw DMcError("Didn't get the right amount of data.");
    }
    ...
    int cmapsize = header->CoMapType ? (header->CoSize / 8) * ((header->Length_hi << 8) | header->Length_lo) : 0;
    ...
    if (header->ImgType == TGA_MAP) chan = header->CoSize / 8;
...
DMC_DECL bool colors_equal(const unsigned char* a, const unsigned char* b, const int chan)
{
    bool same = true;
    for (int i = 0; i < chan; i++) same = same && (*a++ == *b++);
...
DMC_DECL bool match(const unsigned char* src, int& count, const int chan)
{
...
static int encode_rle(const unsigned char* src0, unsigned char* dest, const int size, const int chan)
{
    const unsigned char* src = src0;
    int dp = 0;
