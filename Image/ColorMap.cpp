...
ColorMap<Pixel_T>::ColorMap(const size_t Size_)
{
    C.resize(Size_);
}
...
template<class Pixel_T>
ColorMap<Pixel_T>::ColorMap(const ColorMap<Pixel_T>& O, const size_t Size_)
{
    if (Size_ < 1 || O.size() < 1)
        return;
...
template<class Pixel_T>
Pixel_T ColorMap<Pixel_T>::operator()(const float s_) const
{
...
template<class Pixel_T>
Pixel_T ColorMap<Pixel_T>::SampleWrapped(const float s_) const
{
...
template<class Pixel_T>
ColorMap<Pixel_T> * ColorMap<Pixel_T>::Copy() const
{
...
template<class Pixel_T>
void ColorMap<Pixel_T>::SetSpan(const size_t iLo_, const size_t iHi_, const Pixel_T &vLo_, const Pixel_T &vHi_)
{
    size_t iLo, iHi;
    Pixel_T vLo, vHi;
    if(iHi_ < iLo_) {
...
template<class Pixel_T>
void ColorMap<Pixel_T>::SetSpan(const float sLo_, const float sHi_, const Pixel_T &vLo_, const Pixel_T &vHi_)
{
    float sLo, sHi;
    Pixel_T vLo, vHi;
    if(sHi_ < sLo_) {
...
template<class Pixel_T>
Pixel_T ColorMap<Pixel_T>::Sample01(const float s) const
{
...
template<class Pixel_T>
void ColorMap<Pixel_T>::RealSetSpan(const size_t iLo, const size_t iHi, const Pixel_T &vLo, const Pixel_T &vHi)
{
