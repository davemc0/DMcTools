...
void DeSpeckle(Image_T& dstImg, const Image_T& srcImg)
{
    ...
    for (int y = 1; y < srcImg.h() - 1; y++) {
        for (int x = 1; x < srcImg.w() - 1; x++) {
            ...
            MinP = Min(MinP, srcImg(x, y - 1));     MaxP = Max(MaxP, srcImg(x, y - 1));
            MinP = Min(MinP, srcImg(x + 1, y - 1)); MaxP = Max(MaxP, srcImg(x + 1, y - 1));
            MinP = Min(MinP, srcImg(x - 1, y));     MaxP = Max(MaxP, srcImg(x - 1, y));
            MinP = Min(MinP, srcImg(x + 1, y));     MaxP = Max(MaxP, srcImg(x + 1, y));
            MinP = Min(MinP, srcImg(x - 1, y + 1)); MaxP = Max(MaxP, srcImg(x - 1, y + 1));
            MinP = Min(MinP, srcImg(x, y + 1));     MaxP = Max(MaxP, srcImg(x, y + 1));
            MinP = Min(MinP, srcImg(x + 1, y + 1)); MaxP = Max(MaxP, srcImg(x + 1, y + 1));
...
KernelImage_T MakeGaussianKernel(const int N, const typename KernelImage_T::PixType::ElType sigma)
{
    ...
    for (int y = -N2; y <= N2; y++) {
        for (int x = -N2; x <= N2; x++) {
            typename KernelImage_T::PixType::ElType G = Gaussian2(static_cast<typename KernelImage_T::PixType::ElType>(x), static_cast<typename KernelImage_T::PixType::ElType>(y), sigma);
