...
// The obstacle is that MSVC doesn't support partial specialization.
...
public:
    ...
    {
        channel_cast((*this)[0], p.Luminance());
    }
...
#else
// Specializations for all one-channel pixels.

template<typename _ElType> 
_ElType tPixel<_ElType, 1>::sum_chan() const
    {
    }

// Only one-channel pixels have this:
// Treat the pixel as its first element.
template<class _ElType, int A> 
tPixel<_ElType, A>::operator _ElType() { return (*this)[0]; }

template<class _ElType> 
tPixel<_ElType, 1>::operator _ElType() const { return (*this)[0]; }

// Convert any kind of pixel to this kind.
template<class _ElType, class _ElTypeSrc>
tPixel<_ElType, 1>::tPixel<_ElType, 1>(const tPixel<_ElTypeSrc,1> &p)
{ channel_cast((*this)[0], p[0]); }
template<class _ElType, class _ElTypeSrc>
tPixel<_ElType, 1>::tPixel<_ElType, 1>(const tPixel<_ElTypeSrc,2> &p)
{ channel_cast((*this)[0], p[0]); }
template<class _ElType, class _ElTypeSrc>
tPixel<_ElType, 1>::tPixel<_ElType, 1>(const tPixel<_ElTypeSrc,3> &p)
{ channel_cast((*this)[0], p.Luminance()); }
template<class _ElType, class _ElTypeSrc>
tPixel<_ElType, 1>::tPixel<_ElType, 1>(const tPixel<_ElTypeSrc,4> &p)
{ channel_cast((*this)[0], p.Luminance()); }

// Writable luminance component.
template<class _ElType>
_ElType &tPixel<_ElType, 1>::l() {return (*this)[0];}

// Read-only luminance component.
template<class _ElType>
_ElType tPixel<_ElType, 1>::l() const {return (*this)[0];}

// Read-only luminance component.
template<class _ElType>
_ElType tPixel<_ElType, 1>::lum() const {return (*this)[0];}
#endif
...
public:
    ...
    {
        channel_cast((*this)[0], p[0]);
        channel_cast((*this)[1], p[0]);
        channel_cast((*this)[2], p[0]);
    }
