...
template <class Elem_T>
DMC_DECL void mult_asgn(Elem_T &a, const Elem_T &b) { a *= b; }

// From Alvy Ray Smith's Principles of Image Compositing
template <>
DMC_DECL void mult_asgn<unsigned char>(unsigned char &a, const unsigned char &b)
{
...
template <>
DMC_DECL void mult_asgn<unsigned short>(unsigned short &a, const unsigned short &b)
{
...
public:
    ...
    {
        if (element_traits<In_T>::bytes == element_traits<Out_T>::bytes &&
            element_traits<In_T>::normalized == element_traits<Out_T>::normalized &&
        ...
        else if(!element_traits<In_T>::floating_point && !element_traits<Out_T>::floating_point) // one is normalized but neither is float: unsigned char -> int, etc.
            d = static_cast<Out_T>(s);
...
template <> DMC_DECL void basePixel::channel_cast(unsigned short &d, const unsigned char &s) {unsigned short t=s; d=(t<<8)|t;}
...
public:
    ...
    DMC_DECL Elem_T &g() { ASSERT_D(Chan_>1); return (*this)[1]; }
    DMC_DECL Elem_T &b() { ASSERT_D(Chan_>2); return (*this)[2]; }
    DMC_DECL Elem_T &a() { ASSERT_D(Chan_>3); return (*this)[3]; }

    // Read-only.
    DMC_DECL Elem_T r() const { return (*this)[0]; }
    DMC_DECL Elem_T g() const { ASSERT_D(Chan_>1); return (*this)[1]; }
    DMC_DECL Elem_T b() const { ASSERT_D(Chan_>2); return (*this)[2]; }
    DMC_DECL Elem_T a() const { ASSERT_D(Chan_>3); return (*this)[3]; }
...
DMC_DECL tPixel<Elem_T, Chan_> func(const tPixel<Elem_T, Chan_> &p, Pred_F fnc)
{
...
template<class Elem_T, int Chan_>
DMC_DECL tPixel<Elem_T, Chan_> operator-(const tPixel<Elem_T, Chan_> &p)
{
...
template<class Elem_T, int Chan_>
DMC_DECL bool operator==(const tPixel<Elem_T, Chan_> &p1, const tPixel<Elem_T, Chan_> &p2)
{
    bool eq = true;
...
// With two pixels
template<class Elem_T, int Chan_>
DMC_DECL tPixel<Elem_T, Chan_> operator+(const tPixel<Elem_T, Chan_> &p1, const tPixel<Elem_T, Chan_> &p2)
{
...
template<>
DMC_DECL tPixel<float, 1> operator*(const tPixel<float, 1> &p1, const tPixel<float, 1> &p2)
{
...
// With a left scalar
template<class Elem_T, int Chan_>
DMC_DECL tPixel<Elem_T, Chan_> operator+(const Elem_T &s, const tPixel<Elem_T, Chan_> &p)
{
...
// With a right scalar
template<class Elem_T, int Chan_>
DMC_DECL tPixel<Elem_T, Chan_> operator+(const tPixel<Elem_T, Chan_> &p, const Elem_T &s)
{
...
DMC_DECL Pixel_T LinearInterp(const Pixel_T &p1, const Pixel_T &p2, typename Pixel_T::FloatMathType weight)
{
...
// Channel-wise max.
template<class Elem_T, int Chan_>
DMC_DECL tPixel<Elem_T, Chan_> Max(const tPixel<Elem_T, Chan_> &p1, const tPixel<Elem_T, Chan_> &p2)
{
    tPixel<Elem_T, Chan_> r;
...
// Channel-wise min.
template<class Elem_T, int Chan_>
DMC_DECL tPixel<Elem_T, Chan_> Min(const tPixel<Elem_T, Chan_> &p1, const tPixel<Elem_T, Chan_> &p2)
{
    tPixel<Elem_T, Chan_> r;
...
template<class Elem_T, int Chan_>
DMC_DECL tPixel<Elem_T, Chan_> Abs(const tPixel<Elem_T, Chan_> &p)
{
    tPixel<Elem_T, Chan_> r;
...
template<class Elem_T, int Chan_>
DMC_DECL tPixel<Elem_T, Chan_> CompMult(const tPixel<Elem_T, Chan_> &p1, const tPixel<Elem_T, Chan_> &p2)
{
    tPixel<Elem_T, Chan_> r;
...
template<class Elem_T, int Chan_>
DMC_DECL tPixel<Elem_T, Chan_> CompDiv(const tPixel<Elem_T, Chan_> &p1, const tPixel<Elem_T, Chan_> &p2)
{
    tPixel<Elem_T, Chan_> r;
    for(int i=0; i<Chan_; i++) r[i] = p1[i] / p2[i];
    return r;
...
// Channel-wise clamp.
template<class Elem_T, int Chan_>
DMC_DECL tPixel<Elem_T, Chan_> Clamp(const tPixel<Elem_T, Chan_> &cmin, const tPixel<Elem_T, Chan_> &d, 
...
template<class Elem_T, int Chan_>
DMC_DECL typename tPixel<Elem_T, Chan_>::MathType DiffSqr(const tPixel<Elem_T, Chan_> &A, const tPixel<Elem_T, Chan_> &B)
{
...
template<class Elem_T, int Chan_>
DMC_DECL bool isFinite(const tPixel<Elem_T, Chan_> &p)
{
    for(int i=0; i<Chan_; i++)
        if (!dmcm::isFinite(p[i]) || dmcm::isNaN(p[i]))
            return false;
    return true;
}
...
// Print out the pixel.
template<class Elem_T, int Chan_>
DMC_DECL std::ostream &operator<<(std::ostream &out, const tPixel<Elem_T, Chan_> &p)
{
    out << '[';
    for(int i=0; i<Chan_; i++) {
...
// Read in the pixel.
template<class Elem_T, int Chan_>
DMC_DECL std::istream& operator>>(std::istream& is, tPixel<Elem_T, Chan_>& p)
{
    char st;
    for(int i=0; i<Chan_; i++) {
        is >> st >> p[i];
