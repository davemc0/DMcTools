...
int fwritergbe(RGBE* scanline, int len, FILE* fp) /* write out a rgbe scanline */
{
    ...
    if (len < MINELEN || len > MAXELEN) /* OOBs, write out flat */
        return (int(fwrite((char*)scanline, int(sizeof(RGBE)), len, fp)) - len);
    /* put magic header */
    putc(2, fp);
    putc(2, fp);
    putc(len >> 8, fp);
...
int freadrgbe(RGBE* scanline, int len, FILE* fp) /* read in an encoded rgbe scanline */
{
    ...
    if (len < MINELEN || len > MAXELEN) return (oldreadrgbe(scanline, len, fp));
    ...
    if ((scanline[0][BLU] << 8 | i) != len) return (-1); /* length mismatch! */
    /* read each component */
    for (i = 0; i < 4; i++)
        for (j = 0; j < len;) {
...
int fwritescan(COLOR* scanline, RGBE* clrscan, int len, FILE* fp) /* write out a scanline */
{
    ...
    n = len;
    while (n-- > 0) {
        setrgbe(sp[0], scanline[0][RED], scanline[0][GRN], scanline[0][BLU]);
        scanline++;
        sp++;
    }
...
{
    if (freadrgbe(clrscan, len, fp) < 0) return (-1);
    /* convert scanline */
    rgbe_color(scanline[0], clrscan[0]);
    while (--len > 0) {
        scanline++;
        clrscan++;
        if (clrscan[0][RED] == clrscan[-1][RED] && clrscan[0][GRN] == clrscan[-1][GRN] && clrscan[0][BLU] == clrscan[-1][BLU] &&
...
DMC_DECL int setrgbe(RGBE rgbe, double r, double g, double b) /* assign a short color value */
{
    double d;
    int e;
...
DMC_DECL int rgbe_color(COLOR col, RGBE rgbe) /* convert short to float color */
{
    float f;
