...
static int FileEditComment(char* TempFileName, char* Comment, int CommentSize)
{
    ...
    if (file == NULL) { ErrFatal("could not open temp file for read"); }

    // Read the file back in.
    CommentSize = (int)fread(Comment, 1, 999, file);
...
static void DoCommand(const char* FileName, int ShowIt)
{
    ...
    if (!TempName[0]) { ErrFatal("Cannot find available temporary file name"); }
...
static int CheckFileSkip(void)
{
    ...
    if (ExifOnly) {
        ...
        if (FindSection(M_EXIF) == NULL) { return TRUE; }
    }
...
static void DoFileRenaming(const char* FileName)
{
    ...
    for (a = 0;; a++) {
        ...
        if (!EnsurePathExists(NewBaseName)) { break; }
...
static int DoAutoRotate(const char* FileName)
{
    if (ImageInfo.Orientation >= 2 && ImageInfo.Orientation <= 8) {
        ...
        if (!ZeroRotateTagOnly) {
            ...
            if (Argument == NULL) { ErrFatal("Orientation screwup"); }
...
static int RegenerateThumbnail(const char* FileName)
{
    ...
    sprintf(ThumbnailGenCommand, "mogrify -thumbnail %dx%d \"%s\"", RegenThumbnail, RegenThumbnail, FileName);
...
static void ProcessFile(const char* FileName)
{
    ...
    if (ApplyCommand || AutoRotate) {
        ...
        if (AutoRotate) {
            if (DoAutoRotate(FileName)) { Modified = TRUE; }
        } else {
            struct stat dummy;
    ...
    if (DoModify) { ReadMode |= READ_IMAGE; }
    ...
    if (ShowConcise) {
        ...
        if (!(DoModify) || ShowTags) {
...

            {
                ...
                if (IptcSection) { show_IPTC(IptcSection->Data, IptcSection->Size); }
            }
            printf("\n");
        }
    }
    ...
    if (ThumbSaveName) {
        ...
        if (SaveThumbnail(OutFileName)) { printf("Created: '%s'\n", OutFileName); }
    }
    ...
    if (RegenThumbnail) {
        if (RegenerateThumbnail(FileName)) { Modified = TRUE; }
    }
    ...
    if (ThumbInsertName) {
        ...
        if (ReplaceThumbnail(ThumbFileName)) { Modified = TRUE; }
    } else if (TrimExif) {
        // Deleting thumbnail is just replacing it with a null thumbnail.
        if (ReplaceThumbnail(NULL)) { Modified = TRUE; }
    }
    ...
    if (
        ...
        if (!Modified) { printf("Comment not modified\n"); }
    }
    ...
    if (ExifTimeAdjust || ExifTimeSet || DateSetChars || FileTimeToExif) {
        if (ImageInfo.numDateTimeTags) {
            ...
            if (ExifTimeSet) {
                ...
                if (FileTimeToExif) { FileTimeAsString(ImageInfo.DateTime); }
                if (DateSetChars) {
    ...
    if (RenameToDate) { DoFileRenaming(FileName); }
    DiscardData();
    return;
badtime:
...
static time_t ParseCmdDate(char* DateSpecified)
{
    ...
    UnixTime = mktime(&tm);
    if (UnixTime == -1) { ErrFatal("Specified time is invalid or out of range"); }
...
int main(int argc, char** argv)
{
    ...
    for (argn = 1; argn < argc; argn++) {
        ...
        if (!strcmp(arg, "-te")) {
            ...
            if (RegenThumbnail > 320) { ErrFatal("Specified thumbnail geometry too big!"); }
            DoModify |= MODIFY_JPEG;
            ...
            minutes = seconds = 0;
            if (arg[3] != '-' && arg[3] != '+') { ErrFatal("Error: -ta must be followed by +/- and a time"); }
            n = sscanf(arg + 4, "%d:%d:%d", &hours, &minutes, &seconds);

            if (n < 1) { ErrFatal("Error: -ta must be immediately followed by time"); }
            ...
            if (a < 4 || a > 10) { ErrFatal("Date must be in format YYYY, YYYY:MM, or YYYY:MM:DD"); }
            DateSetChars = a;
            DoModify |= MODIFY_JPEG;
    ...
    if (argn == argc) { ErrFatal("No files to process.  Use -h for help"); }
    ...
    for (; argn < argc; argn++) {
        ...
        if (!FilesMatched) { fprintf(stderr, "Error: No files matched '%s'\n", argv[argn]); }
    }
