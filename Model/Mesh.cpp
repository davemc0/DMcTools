...
DMC_DECL bool CheckVec(const f3vec& V)
{
    ...
    return (len2 < 0.97 || len2 > 1.03 || !std::isfinite(V.x) || !std::isfinite(V.y) || !std::isfinite(V.z));
}
...
void Mesh::GenTexCoords()
{
    ...
    for (AVertex* V = (AVertex*)Verts; V; V = (AVertex*)V->next) {
        if (abs(V->Tex.x) > 1e5 || abs(V->Tex.y) > 1e5 || !std::isfinite(V->Tex.x) || !std::isfinite(V->Tex.y)) WARN_D(1, "GenTex: BadTex: " << Name << V->Tex);
    }
