...
void Mesh::ImportTriObject(const TriObject &Ob, f3Vector::ElType MeshDistFactor,
...
void Mesh::GenColorsFromFaceColors()
{
    ...
    for(Vertex *V = Verts; V; V = V->next) {
        ...
        for(size_t k=1; k<V->Faces.size(); k++) {
            AccNorm += static_cast<AFace *>(V->Faces[k])->Col;
        }
...
DMC_DECL f3Vector ComputeFaceTangent(const f3Vector &P1, const f3Vector &P2, const f3Vector &P3,
                                     const f3Vector &T1, const f3Vector &T2, const f3Vector &T3, const f3Vector &N)
{
    f3Vector Vec1 = P3 - P2;
    f3Vector Vec2 = P1 - P2;
...
void Mesh::GenFaceTangents()
{
    ...
    for(AFace *F = (AFace *)Faces; F; F = (AFace *)F->next) {
        F->Tan = ComputeFaceTangent(F->v0->V, F->v1->V, F->v2->V,
            static_cast<AVertex *>(F->v0)->Tex, static_cast<AVertex *>(F->v1)->Tex, static_cast<AVertex *>(F->v2)->Tex, F->Nor);
...
void Mesh::ApplyTextureTransform(Matrix44<typename f3Vector::ElType> &Mat)
{
    ...
    for(Vertex *V = Verts; V; V = V->next) {
        static_cast<AVertex *>(V)->Tex = Mat * static_cast<AVertex *>(V)->Tex;
    }
}
