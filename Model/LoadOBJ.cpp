...
namespace {
    ...
    {
        f3Vector::ElType x, y;
        if (2 != sscanf(buf,"%f %f",&x,&y)) {
            ASSERT_RM(0,"LoadObj parse failure in Get2D");
        }
    ...
    DMC_DECL f3Vector Get3D(char* buf)
    {
        ...
        if (3 != sscanf(buf,"%f %f %f",&x,&y,&z)) {
            ASSERT_RM(0,"LoadObj parse failure in Get3D");
        }
    ...
    DMC_DECL bool GetFace(char *OBuf, vector<f3Vector> &tverts, vector<f3Vector> &tnormals,
        vector<f3Vector> &ttexcoords, vector<f3Vector> &verts, vector<f3Vector> &normals,
        vector<f3Vector> &texcoords)
    {
