        ...
        for(size_t i = 0; i < (int)Ob->indices.size(); i += 3) {
            ...
            for(int j = 0; j < 3; j++) {
                ...
                if(DoTexcoords)
                    ...
                    fprintf(out, "/");
                if(DoVertexNormals)
                    fprintf(out, "/%lld", normalofs + vi + 1);
                if(DoFaceNormals)
                    fprintf(out, "/%lld", normalofs + Ob->fnindices[i / 3] + 1);
                fprintf(out, " ");
            }
            fprintf(out, "\n");
        }
