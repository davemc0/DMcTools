...
struct BaseObject {
    ...
    Matrix44<f3vec> Transform;    // The current transformation.
    Matrix44<f3vec> TexTransform; // The current texture transformation.
    Aabb Box;
    f3vec dcolor; // The object-wide colors.
    f3vec::ElType alpha;
    f3vec scolor, ecolor, acolor;
    f3vec::ElType shininess, creaseAngle;                                                // Used for later generation of normals.
    ...
    f3vec::ElType RenderPriority;                                                        // Use this to order the objects in some way. Used by operator<.
    ...
    virtual void ApplyTransform(Matrix44<f3vec>& Mat) = 0;

    // Transform all texcoords by this matrix.
    virtual void ApplyTextureTransform(Matrix44<f3vec>& Mat) = 0;
};
