...
void WritableVRMLTriObject::writeVector(const f3Vector& V)
{
...
void WritableVRMLTriObject::writeMaterials()
{
    ...
    {
        indent();
        fprintf(out, "diffuseColor [\n");
        IncIndent();
        for (int i = 0; i < (int)dcolors.size(); i++) {
            indent();
            writeVector(dcolors[i]);
    ...
    if (alphas.size() == dcolors.size()) {
        ...
        // Overall alpha
        f3Vector::ElType Alpha = 0; // This is lame.
        if (alphas.size() == 1) Alpha = 1 - alphas[0];
...
void WritableVRMLTriObject::writeTexCoords()
{
    if (texcoords.size()) {
        ...
        for (int i = 0; i < (int)texcoords.size(); i++) {
            indent();
            fprintf(out, "%0.3f %0.3f", texcoords[i].x, texcoords[i].y);
...
void WritableVRMLTriObject::writeIndices()
{
    if (verts.size()) {
        ...
        for (int i = 0; i < (int)verts.size();) {
            indent();
            for (int j = 0; j < ((PrimType == L_TRIANGLES) ? 3 : 4); j++) fprintf(out, "%d, ", i++);
            if (i == (int)verts.size())
                fprintf(out, "-1]\n\n");
            else
                fprintf(out, "-1,\n");
...
void WritableVRMLMesh::writeVector(const f3Vector& V) { fprintf(out, "%0.5f %0.5f %0.5f", V.x, V.y, V.z); }
...
{
    if (!((VertexType & OBJ_COLORS) || DColorValid || SColorValid || EColorValid || AColorValid || ShininessValid || AlphaValid)) return;
...
void WritableVRMLMesh::writeTexCoords()
{
    if (VertexType & OBJ_TEXCOORDS) {
        ...
        for (AVertex* V = (AVertex*)Verts; V; V = (AVertex*)V->next) {
            indent();
            fprintf(out, "%0.3f %0.3f", V->Tex.x, V->Tex.y);
            if (V->next)
                fprintf(out, ",\n");
            else
                fprintf(out, "]\n\n");
...
void WritableVRMLMesh::writeIndices()
{
    ...
    int ind = 0, i;
    for (Vertex* V = Verts; V; V = V->next, ind++) VIndices[V] = ind;
