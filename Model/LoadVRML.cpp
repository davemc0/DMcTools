...
#define NUM_CYL_SIDES 32
    ...
    for (i = 1; i < NUM_CYL_SIDES; i++) {
        ...
        f3Vector P(cos(M_PI * 2.0 * t) * S.ObjRadius, sin(M_PI * 2.0 * t) * S.ObjRadius, S.ObjHeight * 0.5);
        S.verts.push_back(P);
...
void s_EndCube()
{
    ...
    S.VertexIndices.clear();
    int listi[] = {1, 3, 7, 5, -1, 0, 2, 6, 4, -1, 4, 5, 7, 6, -1, 0, 1, 3, 2, -1, 2, 3, 7, 6, -1, 0, 1, 5, 4, -1};

    for (int i = 0; i < 5 * 3 * 2; i++) S.VertexIndices.push_back(listi[i]);
...
void s_Shininesses(vector<f3Vector::ElType>* V)
{
    ...
    S.shininess = 0;
    int i;
    for (i = 0; i < (int)V->size(); i++) S.shininess += (*V)[i];
    S.shininess /= f3Vector::ElType(i);
    S.shininess *= 128.0; // Convert to OpenGL style.
    if (S.shininess >= 1.0 && S.shininess <= 127.0) S.ShininessValid = true;
}
...
static int DoVertex(Mesh& B, YYObject& S, vector<AVertex*>& VertPtrs, int cii, int vic, int FaceNum, AFace* FF)
{
    ...
    if ((int)S.TexCoordIndices.size() > 0) {
        ASSERT_RM(S.TexCoordIndices[cii] >= 0 && S.TexCoordIndices[cii] < (int)S.texcoords.size(), "texcoord index out of range");
    ...
    case PER_VERTEX_INDEXED:
        ...
        ASSERT_RM(S.NormalIndices[cii] >= 0 && S.NormalIndices[cii] < (int)S.normals.size(), "normal index out of range");
    ...
    case PER_FACE_INDEXED:
        ...
        ASSERT_RM(S.NormalIndices[FaceNum] >= 0 && S.NormalIndices[FaceNum] < (int)S.normals.size(), "normal face index out of range");
    ...
    case OVERALL:
        ...
        break;
    default: ASSERT_RM(0, "Unknown normal binding."); break;
    }
    ...
    case PER_VERTEX_INDEXED:
        ...
        ASSERT_RM(S.MaterialIndices[cii] >= 0 && S.MaterialIndices[cii] < (int)S.dcolors.size(), "Dcolor index out of range");
    ...
    case PER_FACE_INDEXED:
        ...
        ASSERT_RM(S.MaterialIndices[FaceNum] >= 0 && S.MaterialIndices[FaceNum] < (int)S.dcolors.size(), "Dcolor face index out of range");
        FF->Col = S.dcolors[S.MaterialIndices[FaceNum]];
        B.FaceType = B.FaceType | OBJ_COLORS;
        break;
    case OVERALL:
        if ((int)S.dcolors.size() > 0) B.dcolor = S.dcolors[0];
        if ((int)S.alphas.size() > 0) B.alpha = S.alphas[0];
        break;
    default: ASSERT_RM(0, "Unknown dcolor binding."); break;
    }
...
void s_OutputIndexedFaceSet()
{
    ...
    *(BaseObject*)Bp = *((BaseObject*)&S);
    ...
    if (S.MaterialBinding == PER_VERTEX) { ASSERT_R((int)S.dcolors.size() == N); }
    if (S.NormalBinding == PER_VERTEX) { ASSERT_R((int)S.normals.size() == N); }
    if ((int)S.TexCoordIndices.size() == (int)S.VertexIndices.size()) B.VertexType = B.VertexType | OBJ_TEXCOORDS;

    // Create the vertices and add them to the mesh.
    vector<AVertex*> VertPtrs; // Map vertex indices to vertex structs.
    VertPtrs.resize(N);
    int i;
    for (i = 0; i < N; i++) { VertPtrs[i] = NULL; }
    ...
    if (S.TexPtr && (int)S.TexCoordIndices.size() > 0)
        ASSERT_RM((int)S.TexCoordIndices.size() == (int)S.VertexIndices.size(), "Wrong num. of texcoord indices.");

    if ((int)S.alphas.size() > 0) ASSERT_RM(S.alphas.size() == S.dcolors.size(), "Inconsistent alpha list.");
    ...
    if ((ReqAttribs & OBJ_COLORS) && (S.MaterialBinding == PER_FACE || S.MaterialBinding == PER_FACE_INDEXED)) { B.GenColorsFromFaceColors(); }
    if ((ReqAttribs & OBJ_NORMALS) && (S.NormalBinding == PER_FACE || S.NormalBinding == PER_FACE_INDEXED)) { B.GenNormalsFromFaceNormals(); }
    if (ReqAttribs & OBJ_TEXCOORDS) { B.GenTexCoords(); }
    ...
    if (B.VertexType & OBJ_NORMALS) {
        ...
        for (AVertex* V = (AVertex*)B.Verts; V; V = (AVertex*)V->next) {
            if (!Finite(V->Nor.x)) cerr << "LoadVRML: Failed normal check: " << V->Nor << endl;
            f3Vector::ElType len2 = V->Nor.length2();
            if (len2 < 0.97 || len2 > 1.03) cerr << "LoadVRML: Failed normal check: " << V->Nor << endl;
        }
    }
    ...
    if (B.VertexType & OBJ_TEXCOORDS) {
        ...
        // Sanity check the normals.
        for (AVertex* V = (AVertex*)B.Verts; V; V = (AVertex*)V->next) {
            if (!Finite(V->Tex.x) || !Finite(V->Tex.y)) cerr << "LoadVRML: BadTex: " << V->Tex << endl;
            if (Abs(V->Tex.x) > 1e5 || Abs(V->Tex.y) > 1e5) cerr << "LoadVRML: BadTex: " << V->Tex << endl;
        }
    }
#endif
...
bool Model::LoadVRML(const char* fname, const unsigned int RequiredAttribs, const unsigned int AcceptedAttribs)
{
