...
void s_Tex2Tran_begin()
{
    ...
    S.Center = Vector(0,0,0);
    S.Scale = Vector(1,1,1);
}
...
void s_Transform_begin()
{
    ...
    S.ScaleOrientationAxis = Vector(0,0,1);
    S.Translation = Vector(0,0,0);
    S.Center = Vector(0,0,0);
    S.Scale = Vector(1,1,1);
}
...
void s_LightParam1(float f, int p)
{
...
void s_LightParam3(Vector *v, int p)
{
    LightInfo *L = LitDB.LightList.back();
    if(p == 3)
        L->Color = *v;
...
void s_CameraParam1(float f, int p)
{
...
void s_CameraParam3(Vector *v, float f, int p)
{
    CameraInfo *L = CamDB.CameraList.back();
    if(p == 1)
        L->Position = *v;
