...
template <class Fl_T> class Matrix44 {
public:
    Matrix44()
    {
    ...
    t3Vector<Fl_T> operator*(const t3Vector<Fl_T>& right) const { return Project(right); }
    ...
    template <class InFl_T> void Set(InFl_T* in)
    {
        InFl_T* p = in;
        Fl_T* m = reinterpret_cast<Fl_T*>(mat);
        for (int i = 0; i < 16; i++) m[i] = static_cast<Fl_T>(*p++);
        inverse_valid = false;
        is_identity = false;
    }
    ...
    template <class InFl_T> void SetTranspose(InFl_T* in)
    {
        InFl_T* p = in;
        for (int i = 0; i < 4; i++)
            for (int j = 0; j < 4; j++) mat[j][i] = static_cast<Fl_T>(*p++);
        inverse_valid = false;
        is_identity = false;
    }
    ...
    Fl_T& operator()(const int r, const int c) { return mat[r][c]; }
    ...
    void SetSingularityThreshold(const Fl_T sing) { singularity_thresh = sing; }
...
template <class Fl_T> DMC_DECL Matrix44<Fl_T> MScale(const t3Vector<Fl_T>& V)
{
    Matrix44<Fl_T> M;
    M.Scale(V);
    return M;
}
template <class Fl_T> DMC_DECL Matrix44<Fl_T> MScale(const Fl_T s)
{
    Matrix44<Fl_T> M;
    M.Scale(t3Vector<Fl_T>(s, s, s));
    return M;
}
template <class Fl_T> DMC_DECL Matrix44<Fl_T> MRotate(const Fl_T angRad, const t3Vector<Fl_T>& axis)
{
    Matrix44<Fl_T> M;
    M.Rotate(angRad, axis);
    return M;
}
template <class Fl_T> DMC_DECL Matrix44<Fl_T> MTranslate(const t3Vector<Fl_T>& V)
{
    Matrix44<Fl_T> M;
    M.Translate(V);
    return M;
}
template <class Fl_T> DMC_DECL Matrix44<Fl_T> MFrustum(Fl_T l, Fl_T r, Fl_T b, Fl_T t, Fl_T zn, Fl_T zf)
{
    Matrix44<Fl_T> M;
    M.Frustum(l, r, b, t, zn, zf);
    return M;
}
template <class Fl_T> DMC_DECL Matrix44<Fl_T> MPerspective(Fl_T fovyRad, Fl_T aspect, Fl_T znear, Fl_T zfar)
{
    Matrix44<Fl_T> M;
    M.Perspective(fovyRad, aspect, znear, zfar);
    return M;
}
template <class Fl_T> DMC_DECL Matrix44<Fl_T> MLookAt(const t3Vector<Fl_T>& eye, const t3Vector<Fl_T>& lookat, const t3Vector<Fl_T>& up)
{
    Matrix44<Fl_T> M;
...
template <class Fl_T> DMC_DECL std::ostream& operator<<(std::ostream& os, const Matrix44<Fl_T>& m)
{
    os << m.string();
    return os;
}

template <class Fl_T> DMC_DECL std::istream& operator>>(std::istream& is, Matrix44<Fl_T>& m)
{
    char st;
    Fl_T mat[4][4];
