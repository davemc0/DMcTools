...
DMC_DECL bool chance(int n, int out_of)
{
	return (out_of > 0) ? ((rand() % out_of) < n) : false;
}

// A random number on [0 .. out_of)
DMC_DECL int randn(int out_of)
{
    return (out_of > 0) ? (rand() % out_of) : 0;
}
...
template<class Elem_T> DMC_DECL Elem_T TRand(const Elem_T low, const Elem_T high) { return low + TRand<Elem_T>() * (high - low); } // A random number on low to high
...
template<class T>
DMC_DECL T Gaussian2(const T x, const T y, const T sigma)
{
...
// Return a random number with a normal distribution
DMC_DECL double NRand(const double sigma = 1.0, const double mu = 0.0) { return TNRand<double>(sigma) + mu; }

// Return a random number with a normal distribution
DMC_DECL float NRandf(const float sigma = 1.0f, const float mu = 0.0f) { return TNRand<float>(sigma) + mu; }

// True approximately frac percent of the time
DMC_DECL bool chance(float frac)
{
	return frac > DRandf();
}
