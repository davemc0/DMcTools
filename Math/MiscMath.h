...
#define SCALARTMPLMINMAX(TEMPLATE, R, T, MIN, MAX)                       \
    TEMPLATE DMC_DECL R min(T a, T b) { return MIN; }                    \
    TEMPLATE DMC_DECL R max(T a, T b) { return MAX; }                    \
    TEMPLATE DMC_DECL R min(T a, T b, T c) { return min(min(a, b), c); } \
    TEMPLATE DMC_DECL R max(T a, T b, T c) { return max(max(a, b), c); } \
    TEMPLATE DMC_DECL R clamp(T v, T lo, T hi) { return min(max(v, lo), hi); }
