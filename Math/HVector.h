...
template <class Elem_T> class HVector {
public:
    ...
    DMC_DECL HVector<Elem_T> operator*(const Elem_T d) const
    {
        ...
        for (size_t i = 0; i < size(); i++) b[i] = V[i] * d;
    ...
    {
        for (size_t i = 0; i < size(); i++) V[i] = V[i] * d;
    ...
    DMC_DECL HVector<Elem_T> operator+(const HVector<Elem_T>& A) const
    {
        ...
        HVector<Elem_T> b(size());
        for (size_t i = 0; i < size(); i++) b[i] = V[i] + A[i];
    ...
    {
        for (size_t i = 0; i < size(); i++) V[i] = static_cast<Elem_T>(0);
    ...
    DMC_DECL Elem_T lenSqr() const
    {
        ...
        for (size_t i = 0; i < size(); i++) { l2 += Sqr(V[i]); }
...
template <class Elem_T> std::istream& operator>>(std::istream& os, HVector<Elem_T>& p);
