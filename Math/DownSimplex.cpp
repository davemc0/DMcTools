...
template <class Elem_T> DMC_DECL HVector<Elem_T> ComputePSum(HVector<Elem_T>* p, size_t ndim)
{
    HVector<Elem_T> psum(ndim);
    psum.zero();

    // Loop over the simplex, so ndim+1.
    for (size_t i = 0; i <= ndim; i++) { psum += p[i]; }
...
template float DownSimplex(HVector<float>* p, size_t ndim, float ftol, float (*funk)(const HVector<float>&, void*), void* UserData, int& nfunk, const int NMax);

template double DownSimplex(HVector<double>* p, size_t ndim, double ftol, double (*funk)(const HVector<double>&, void*), void* UserData, int& nfunk,
                            const int NMax);
