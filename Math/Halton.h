...
DMC_INLINE std::vector<float> Hammersley(int i, int dim, int Count)
{
    ASSERT_D(dim<=10);
...
DMC_INLINE std::vector<float> Halton(int i, int dim)
{
    ASSERT_D(dim<=10);
    std::vector<float> T(dim);
    for(int k=0; k<dim; k++) T[k] = 0;
...
public:
    ...
    inline void Get(float &vx, float &vy, const int index)
    {
