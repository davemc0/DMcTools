...
std::string HVector<Elem_T>::print() const
{
    std::string s;
    char ii[32];
...
template<class Elem_T>
HVector<Elem_T> & HVector<Elem_T>::rand()
{
    for (size_t i = 0; i<size(); i++)
        (*this)[i] = TRand<Elem_T>();
...
template<class Elem_T>
HVector<Elem_T> & HVector<Elem_T>::nrand(const Elem_T sigma)
{
    for (size_t i = 0; i<size(); i++)
		(*this)[i] = NRand(sigma);
...
template<class Elem_T>
HVector<Elem_T> Mean(const HVector<Elem_T> *VA, const size_t count)
{
    ASSERT_R(count > 0);
...
template<class Elem_T>
HMatrix<Elem_T> OuterProd(const HVector<Elem_T> &A, const HVector<Elem_T> &B)
{
...
template<class Elem_T>
HMatrix<Elem_T> Covariance(const HVector<Elem_T> *VA, const size_t count)
{
    ASSERT_R(count > 0);
