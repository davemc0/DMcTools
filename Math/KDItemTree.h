...
template <class Item_T> class KDItem {
public:
    Item_T Value;
    KDItem *lower, *higher;
...
template <class Item_T, int NumDim = 3> class KDItemTree {
    KDItem<Item_T>* Root;
    ...
    DMC_DECL bool within(const f3Vector& a, const f3Vector& b, const f3Vector::ElType threshSq) const { return (a - b).length2() < threshSq; }

public:
    DMC_DECL KDItemTree() { Root = NULL; }
    ...
    DMC_DECL bool empty() const { return Root == NULL; }
...
private:
    ...
    DMC_DECL void rnearest(KDItem<Item_T>* Root, const f3Vector& Qr, f3Vector::ElType& bestdSqr, const Item_T*& bestItem, int lev = 0) const
    {
        if (Root == NULL) return;
        if (lev == 3) lev = 0;
        ...
        f3Vector::ElType distL = (lev == 0) ? (Root->Value.x - Qr.x) : (lev == 1) ? (Root->Value.y - Qr.y) : (Root->Value.z - Qr.z);
        ...
        if (distL > 0) {
            ...
            if (distLSqr < bestdSqr) rnearest(Root->higher, Qr, bestdSqr, bestItem, lev + 1);
        } else {
            ...
            if (distLSqr < bestdSqr) rnearest(Root->lower, Qr, bestdSqr, bestItem, lev + 1);
        }
    }

    // This one is called recursively internally.
    DMC_DECL const Item_T* rfind(const KDItem<Item_T>* Root, const f3Vector& Qr, int lev = 0) const
    {
        if (lev == 3) lev = 0;
    ...
    // This one is called recursively internally.
    DMC_DECL const Item_T* rfindeps(const KDItem<Item_T>* Root, const f3Vector& Qr, f3Vector::ElType thresh, int lev = 0) const
    {
        if (lev == 3) lev = 0;

        if (within(Qr, Root->Value, dmcm::Sqr(thresh))) return &(Root->Value);

        if (myless(Qr, Root->Value, lev)) {
            if (Root->lower) {
                if (const Item_T* Res = rfindeps(Root->lower, Qr, thresh, lev + 1)) return Res;
            }
            // Didn't find in the proper half.
            // Do we need to search the other half?
            if (Root->higher == NULL) return NULL;

            if (((lev == 0) && fabs(Root->Value.x - Qr.x) < thresh) || ((lev == 1) && fabs(Root->Value.y - Qr.y) < thresh) ||
            ...
            if (Root->higher) {
                if (const Item_T* Res = rfindeps(Root->higher, Qr, thresh, lev + 1)) return Res;
            }

            // Didn't find in the proper half.
            // Do we need to search the other half?
            if (Root->lower == NULL) return NULL;
