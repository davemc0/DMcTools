...
template <class Vector_T> class BBox {
    Vector_T Ctr;
...
public:
    ...
    DMC_DECL BBox(const Vector_T& MinV, const Vector_T& MaxV) : valid(true), sphere_valid(false), MinV(MinV), MaxV(MaxV) {}
    ...
    DMC_DECL bool Inside(const Vector_T& P) const { return !((P.x < MinV.x || P.y < MinV.y || P.z < MinV.z || P.x > MaxV.x || P.y > MaxV.y || P.z > MaxV.z)); }
    ...
    {
        ASSERT_D(r >= 0.0);
        return (!(P.x + r < MinV.x || P.y + r < MinV.y || P.z + r < MinV.z || P.x - r > MaxV.x || P.y - r > MaxV.y || P.z - r > MaxV.z));
    }
...
template <class Vector_T> DMC_DECL std::ostream& operator<<(std::ostream& os, const BBox<Vector_T>& b)
{
    os << b.MinV.string() << b.MaxV.string();
    return os;
}

template <class Vector_T> DMC_DECL std::istream& operator>>(std::istream& is, BBox<Vector_T>& b)
{
    is >> b.MinV >> b.MaxV;
    return is;
}
